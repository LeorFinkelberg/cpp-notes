...
```
NB! На практике лучше использовать квалификатор `std::` (или еще говорят полные квалифицированные имена) или объявление `using` для обеспечения доступа только к отдельным именам
```c++
using std::cout;
using std::cin;
using std::endl;
...
Тернарный оператор (записывается также как и в Java)
```c++
int value1 = 42;
int value2 = 88;
